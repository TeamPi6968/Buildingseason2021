...
void Motors::RTPI_SparkMax(bool brushless, int canID, double acceleration, bool inverted, bool encoder, bool pidController) {
  ...
  if(encoder) {
    rtpiSparkMaxEncoder = new SparkMaxRelativeEncoder(rtpiSparkMax->GetEncoder());
  }
  if(pidController) {
    rtpiSparkMaxPIDController = new SparkMaxPIDController(rtpiSparkMax->GetPIDController());
  }
}
...
SparkMaxRelativeEncoder *Motors::GetSparkMaxEncoder() {
  return this->rtpiSparkMaxEncoder;
}

//returns the SparkMax PIDcontroller object of selected motor(s)
SparkMaxPIDController *Motors::GetSparkMaxPIDController() {
