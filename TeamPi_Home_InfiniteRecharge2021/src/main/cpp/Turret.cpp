...
Turret::Turret() 
            ...
            shoot_motor_master.ConfigSelectedFeedbackSensor(FeedbackDevice::IntegratedSensor, 0, 50);
            shoot_motor_slave.ConfigSelectedFeedbackSensor(FeedbackDevice::IntegratedSensor, 0, 50);

            shoot_motor_master.Config_kP(0,0.5);
            shoot_motor_master.Config_kI(0,0);
            shoot_motor_master.Config_kD(0,0.1);

            shoot_motor_slave.Config_kP(0,0.5);
            shoot_motor_slave.Config_kI(0,0);
            shoot_motor_slave.Config_kD(0,0.1);
            }


void Turret::Shoot(bool shoot, float speed_turret){
    if (shoot == true){
        shoot_motor_master.Set(ControlMode::Velocity, speed_turret);
    }
    else if (shoot == false){
        shoot_motor_master.Set(ControlMode::Velocity,0);
    }
}
